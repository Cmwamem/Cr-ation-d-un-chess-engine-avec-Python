#les corrections ont été apportées dans cette version
...
def getBestMove(board, depthArg):

    gameState = defineGameState(board)

    if gameState == "midgame":
        if depthArg == "adaptative":
            start = time.time()
            analyse(board)
            end = time.time()
            timeAnalysis = float(format(end-start))+10**(-15)
            legalMovesInPosition = board.legal_moves.count() + 1
            depth = math.floor(4/3*math.log(25/timeAnalysis)/math.log(legalMovesInPosition))
            if depth > 4:
                depth = 4
        else:
            depth = depthArg
